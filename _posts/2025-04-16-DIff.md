...
<!-- A diffusion model is only different to a flow model in that the deterministic process of flowing become stochastic. Apart from the step in the direction of the vector field, we add a random walk, or more precisely, a Brownian motion. This type of model is particularly important in generative modeling, because it trades off some qualtity in flow model generation to diversity, due to its stochastic nature. -->

To quote from [^6]: "Creating noise from data is easy; creating data from noise is generative modeling." The diffusion models draw inspirations from stochastic differential equations (SDE) in polluting the data with noise, and recover it by denoising. 
